...
class TrajectoryMetricsNode(Node):
    def __init__(self) -> None:
        ...
        #per il debug------
        self.audit_dir = os.path.join(self.output_dir, "debug_audit")
        os.makedirs(self.audit_dir, exist_ok=True)

        self.cam_audit_path = os.path.join(self.audit_dir, "camera_odom_audit.csv")
        self.amb_audit_path = os.path.join(self.audit_dir, "ambulance_odom_audit.csv")

        with open(self.cam_audit_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["t", "x", "y", "yaw", "frame_id", "child_frame_id", "valid"])

        with open(self.amb_audit_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["t", "x", "y", "yaw", "frame_id", "child_frame_id", "valid"])
        #----per il debug
    ...
    def camera_odom_callback(self, msg: Odometry) -> None:
        ...
        if x is None or y is None or yaw is None:
            ...
            with open(self.cam_audit_path, "a", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow([t, x, y, yaw, frame_id, child_frame_id, 0])
        ...
        with open(self.cam_audit_path, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([t, x, y, yaw, frame_id, child_frame_id, 1])
        #------
    ...
    def ambulance_odom_callback(self, msg: Odometry) -> None:
        ...
        if  x is None or y is None or yaw is None:
            with open(self.amb_audit_path, "a", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow([t, x, y, yaw, frame_id, child_frame_id, 0])
